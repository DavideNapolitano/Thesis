...
def StemmLemm(tagg): 
    ...
    word_3=set()
    to_keep=["max","new","net","msg","bad","os","ipc","not","eof","dfs",'io',"up","map","use","bp","nodename","nn","no","src","dst","end","rdd","server"]
...
def RemoveFinalS(input):
    TotWord = []
    for el in tqdm(enumerate(input)):  # ALL DATA
        for wrd in el[1]:
            TotWord.append(wrd)
...
#%%
if Origin:
    f=open(f"Data Folder/{name}/{folder}/{name}_Input_ORIGIN+MESSAGE.txt","w")
else:
    f=open(f"Data Folder/{name}/{folder}/{name}_Input_MESSAGE.txt","w")
...
#%%
# print(len(input_preproc),len(no_special))
# for i,el in enumerate(no_special):
#     if i<30:
#         print(el)
...
#%%
if Origin:
    f=open(f"Data Folder/{name}/{folder}/{name}_Clean_ORIGIN+MESSAGE.txt","w")
else:
    f = open(f"Data Folder/{name}/{folder}/{name}_Clean_MESSAGE.txt", "w")
...
#%%
for i,el in enumerate(stemm2):
    tag=tagg[i]
    tag_w=[el1[0] for el1 in tag]
    changes=[]
    for w in el:
        if w not in tag_w:
            changes.append((w))
...
visualizer = FreqDistVisualizer(features=features1, orient='h', n=30,  size=(1080,720), title=f"CountVectorizer - {name} - Top-30", color="cornflowerblue")
visualizer.fit(X)
visualizer.show()
...
# visualizer = FreqDistVisualizer(features=features3, orient='h', n=50, size=(1080, 1080),title="TfIdfVectorizer - word:word", color="orange")
# visualizer.fit(X3)
# visualizer.show()
...
#%%
if Origin:
    f = open(f"Data Folder/{name}/{folder}/{name}_Input_Output_ORIGIN+MESSAGE.txt", "w")
    f2=open(f"Data Folder/{name}/{folder}/{name}_Raw_ORIGIN+MESSAGE.txt", "w")
else:
    f = open(f"Data Folder/{name}/{folder}/{name}_Input_Output_MESSAGE.txt", "w")
    f2 = open(f"Data Folder/{name}/{folder}/{name}_Raw_MESSAGE.txt", "w")
...
print("SAVE DATA")
if Origin:
    fname = f"Data Folder/{name}/{folder}/{name}_InputData_ORIGIN+MESSAGE.txt"
else:
    fname = f"Data Folder/{name}/{folder}/{name}_InputData_MESSAGE.txt"
